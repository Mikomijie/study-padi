...
export default function Games() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (documentId) {
        // Load flashcards for specific document
        const { data: doc } = await supabase.from("documents").select("title").eq("id", documentId).single();
        if (doc) {
          const { data } = await supabase
            .from("flashcards")
            .select("id, term, definition, difficulty_level")
            .eq("user_id", session.user.id)
            .eq("source", doc.title);
          setFlashcards(
            (data || []).map((f: any) => ({ id: f.id, term: f.term, definition: f.definition, difficulty: f.difficulty_level as any }))
          );
        }
      } else {
  ...
  // No documentId → show document selection
  if (!documentId) {
    return (
      <AppLayout user={mockUser}>
        <div className="container mx-auto px-4 py-6 sm:py-8 max-w-4xl">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-6 sm:mb-8">
            ...
            <p className="text-muted-foreground text-sm sm:text-base">Choose a document to practice with games.</p>
          </motion.div>
          ...
          {documents.length === 0 ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-card p-8 text-center">
              ...
              <p className="text-muted-foreground mb-4 text-sm">Upload a document to generate flashcards and play games.</p>
            ...
            <div className="space-y-3">
              {documents.map((doc, index) => (
                <motion.button
                  ...
                  onClick={() => navigate(`/games/${doc.id}`)}
                  className="w-full glass-card p-4 sm:p-5 text-left hover:border-primary/50 transition-all group"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg gradient-bg-accent flex items-center justify-center flex-shrink-0">
  ...
  // Has documentId but no flashcards
  if (flashcards.length === 0) {
    return (
      <AppLayout user={mockUser}>
        <div className="container mx-auto px-4 py-6 sm:py-8 max-w-4xl">
          <Button variant="ghost" onClick={() => navigate("/games")} className="mb-4 text-muted-foreground min-h-[44px]">
            <ArrowLeft className="w-4 h-4 mr-2" /> Back
          </Button>
          <div className="glass-card p-8 text-center">
            <AlertIcon className="w-12 h-12 text-muted-foreground mx-auto mb-4" />
            <h2 className="text-lg font-semibold mb-2">No flashcards found</h2>
            <p className="text-muted-foreground mb-4 text-sm">This document doesn't have any flashcards yet. Try re-uploading it.</p>
          </div>
        </div>
      </AppLayout>
    );
      ...
      <div className="container mx-auto px-4 py-6 sm:py-8 max-w-4xl">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-6 sm:mb-8">
          {gameMode ? (
            ...
            <>
              <Button variant="ghost" onClick={() => navigate("/games")} className="mb-4 text-muted-foreground min-h-[44px]">
                <ArrowLeft className="w-4 h-4 mr-2" /> Choose Document
              </Button>
              <h1 className="font-display text-2xl sm:text-3xl md:text-4xl font-bold mb-2">
                <span className="gradient-text-primary">Flashcard</span> Games
              </h1>
              <p className="text-muted-foreground text-sm sm:text-base">
                {flashcards.length} flashcards available. Choose a game mode!
              </p>
            </>
          )}
        </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="grid sm:grid-cols-2 gap-4 sm:gap-6">
              {gameInfo.map((game, index) => (
                <motion.button
                  ...
                  className={cn("glass-card p-5 sm:p-6 text-left group hover:border-primary/50 transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] min-h-[120px]")}
                >
                  <div className={cn(
...
function ClassicFlashcards({ flashcards, onComplete }: { flashcards: Flashcard[]; onComplete: () => void }) {
  ...
  const handleKnown = () => { setKnownCount((p) => p + 1); setIsFlipped(false); setTimeout(() => setCurrentIndex((p) => p + 1), 200); };
  const handleReview = () => { setReviewCount((p) => p + 1); setIsFlipped(false); setTimeout(() => setCurrentIndex((p) => p + 1), 200); };
...
function SpeedChallenge({ flashcards, onComplete }: { flashcards: Flashcard[]; onComplete: () => void }) {
  ...
  const handleAnswer = (answer: string) => {
    if (answer === currentCard.definition) { setScore((s) => s + 10); setStreak((s) => s + 1); }
